...
func (r *Repository) AssignWorkOrder(ctx context.Context, id, assignee string, version int, at string) error {
	result, err := r.DB.ExecContext(ctx, "UPDATE work_orders SET assignee_id=?,state='assigned',version=version+1,updated_at=? WHERE id=? AND state='queued' AND version=?", assignee, at, id, version)
	if err != nil {
